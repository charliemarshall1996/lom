...
from gensim.models import LdaModel, CoherenceModel, EnsembleLda
import numpy as np
import pandas as pd
...
def run(data, genre):
    ...
    space = [Integer(5, 100, prior='log-uniform', name="num_topics"),
             ...
             Real(0.01, 1, prior='log-uniform', name='minimum_phi_value'),
             Integer(10, 100, prior='log-uniform', name='iterations'),
             Integer(3, 50, name="num_models"),
             Integer(1, 5, name="min_cores"),
             Integer(1, 5, prior='log-uniform', name='min_samples'),
             Real(0.05, 0.9, prior="log-uniform", name="epsilon"),
             ]

    # Define objective function
    @use_named_args(space)
    def lda_optimizer(**params):
        logger.info("Optimizing with: %s", params)
        model = EnsembleLda(corpus=corpus, id2word=dictionary,
                            dtype=np.float64, random_state=42, **params)
    ...
    result = gp_minimize(lda_optimizer, space, n_calls=256,
...
if __name__ == "__main__":
    df = pd.DataFrame()
    for i, chunk in enumerate(pd.read_csv("./data/raw/song_lyrics_sampled.csv", encoding='utf-8-sig', encoding_errors='ignore', chunksize=100000, usecols=['tag', 'lyrics', 'language_cld3'])):
